            ...
            protected String call() throws Exception {
                ...
                try {
                    ...
                    String command = String.format(
                            ...
                            + "        export LC_ALL=C; zgrep -a -H -n -C 15 -E \"$1\" \"$f\"; "
                            + "      else "
                            + "        export LC_ALL=C; grep -a -H -n -C 15 -E \"$1\" \"$f\"; "
                            + "      fi; "
                            + "    done"
                            + "  ' _ '%2$s' {} + 2>/dev/null; "
                            + "elif test -f '%1$s'; then "
                            + "  if echo '%1$s' | grep -q \"\\.gz$\"; then "
                            + "    export LC_ALL=C; zgrep -a -H -n -C 15 -E '%2$s' '%1$s' 2>/dev/null; "
                            + "  else "
                            + "    export LC_ALL=C; grep -a -H -n -C 15 -E '%2$s' '%1$s' 2>/dev/null; "
                            + "  fi; "
                            + "else "
